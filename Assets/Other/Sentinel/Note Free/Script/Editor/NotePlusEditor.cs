...
namespace Sentinel.NotePlus
{
    public class NotePlusEditor : EditorWindow
    {
        ...
        // Panel
        public enum PanelType { None,NewNote,OpenNote,ImportNote,ExportNote };
        PanelType _panelType;
        NoteData _exportNoteData;
        string _newNoteName = "New Note Name";
        string _newNotePath = Directory.GetCurrentDirectory() + "/Assets";
        string _exportNotePath = Directory.GetCurrentDirectory();
        bool _exportNoteEncryption = true;
        string _importNoteFilePath = Directory.GetCurrentDirectory();
        string _importNoteDataPath = Directory.GetCurrentDirectory() + "/Assets";
        ...
        void ToolArea()
        {
            ...
            switch (_selectedToolArea)
            {
                case AreaType.Background:
                    ...
                    {
                        Texture2D[] icons = EditorIcon.GetIcons(new EditorIconName[6]
                        { EditorIconName.HeaderIcon, EditorIconName.TextAreaIcon, EditorIconName.TodoListIcon, EditorIconName.ColorIcon, EditorIconName.ImageIcon, EditorIconName.VectorIcon });
        ...
        void MenuPanel()
        {
            ...
            GUIStyle importNoteButtonStyle = new GUIStyle(GUI.skin.button);
            GUIStyle exportNoteButtonStyle = new GUIStyle(GUI.skin.button);
            ...
            switch (_panelType)
            {
                ...
                case PanelType.OpenNote:
                    ...
                    break;
                case PanelType.ImportNote:
                    importNoteButtonStyle.fontStyle = FontStyle.Bold;
                    importNoteButtonStyle.fontSize += 2;
                    break;
                case PanelType.ExportNote:
                    exportNoteButtonStyle.fontStyle = FontStyle.Bold;
                    exportNoteButtonStyle.fontSize += 2;
                    break;
            }
            ...
            if (GUILayout.Button("Import Note", importNoteButtonStyle, GUILayout.Width(100), GUILayout.Height(30)))
            { _panelType = PanelType.ImportNote; OnChangePanelType(); }
            if (GUILayout.Button("Exporte Note", exportNoteButtonStyle, GUILayout.Width(100), GUILayout.Height(30)))
            { _panelType = PanelType.ExportNote; OnChangePanelType(); }
            GUILayout.EndHorizontal();
        ...
        void ImportNotePanel()
        {
            _importNoteFilePath = EditorDraw.FileField(_importNoteFilePath, "txt,snpd", "Select Note");
            if (_importNoteFilePath == "")
                return;
            if (!_importNoteFilePath.Contains("txt") & !_importNoteFilePath.Contains("snpd"))
                return;

            string[] data = _importNoteFilePath.Split('/');
            string dataName = data[data.Length - 1].Split('.')[0];

            _importNoteDataPath = EditorDraw.FolderField(_importNoteDataPath, "Path to be created");

            string pathResult = "";
            bool error_WrongFolder = false;
            _errorDebug.Clear();

            if (!_importNoteDataPath.Contains("Assets"))
            {
                error_WrongFolder = true;
                AddError("Please select folder from assets folder.");
            }
            if (!error_WrongFolder)
                pathResult = "Assets" + _importNoteDataPath.Split("Assets")[1] + "/" + dataName + ".asset";
            ...
            if (!error_WrongFolder)
                if (GUILayout.Button("Create Note"))
                {
                    NoteData asset = ScriptableObject.CreateInstance<NoteData>();
                    AssetDatabase.CreateAsset(asset, pathResult);
                    asset.NoteDataImport(_importNoteFilePath);
                    AssetDatabase.SaveAssets();
                    _panelType = PanelType.None;
                    _noteData = asset;
                }
        ...
        void ExportNotePanel()
        {
            if (_exportNoteData == null)
            {
                GUIStyle headerStyle = new GUIStyle(GUI.skin.label);
                headerStyle.alignment = TextAnchor.MiddleCenter;
                GUILayout.Label("SELECT EXPORTE NOTE", headerStyle);
                _exportNoteData = EditorDraw.AllNoteGridButton(position.width);
            }
            else
            {
                ...
                GUILayout.BeginHorizontal();
                GUILayout.Label("Exporte Note", GUILayout.Width(100));
                if (GUILayout.Button("Edit", GUILayout.Width(50)))
                {
                    _exportNoteData = null;
                    GUILayout.EndHorizontal();
                    return;
                }
                GUILayout.Label(" => <color=green>" + _exportNoteData.name + "</color>", infoStyle);
                GUILayout.EndHorizontal();

                _exportNotePath = EditorDraw.FolderField(_exportNotePath);

                GUILayout.BeginHorizontal();
                GUILayout.Label("Encryption", GUILayout.Width(100));
                _exportNoteEncryption = GUILayout.Toggle(_exportNoteEncryption, "");
                GUILayout.EndHorizontal();

                if (GUILayout.Button("Export"))
                {
                    EditorFunction.NoteDataExport(_exportNotePath, _exportNoteData, _exportNoteEncryption);
                    Debug.Log("<color=green>" + _exportNoteData.name + "</color> data exported successfully.File path (" + _exportNotePath + ")");
                    _exportNoteData = null;
                }
            }
        }
